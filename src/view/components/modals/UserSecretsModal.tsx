...
export const UserSecretsModal: FC<BaseModalProps> = ({
    ...
    const [azureDevOpsToken, setAzureDevOpsToken] = useState<string | undefined>(
        undefined,
    )
    ...
    const handleSave = () => {
        if (clockifyApiKey !== undefined)
            dispatch(userSecretsActions.setClockifyApiKey({ clockifyApiKey }))
        if (azureDevOpsToken !== undefined)
            dispatch(userSecretsActions.setAzureDevOpsToken({ azureDevOpsToken }))
        hideModal()
        if (onSave) onSave()
    }
        ...
        >
            <Row>
                <Col xs={24}>
                    ...
                    />
                    <Input.Password
                        placeholder="Azure Dev Ops Token"
